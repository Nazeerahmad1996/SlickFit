...
import { ImageBackground, View, TouchableOpacity, FlatList, Linking, ScrollView } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { LinearGradient } from 'expo-linear-gradient';
import { Container, Text, Body, Right, List, Thumbnail, ListItem, Button } from 'native-base';
...
export default class Diets extends Component {
  static navigationOptions = ({ navigation }) => ({
    title: `${Strings.ST3}`,
    headerRight: <Ionicons name="md-search" style={{ marginRight: 20 }} size={27} color="white" onPress={() => navigation.navigate('CategoriesScreen')} />
  });
    ...
    return (
      <Container style={styles.background_general}>
        ...
        <ScrollView>
          ...
          <Button block info
            onPress={ ()=> Linking.openURL('https://slickfitness.org/add-slick-meal-plan/') }>
            <Text>{Strings.ST115}</Text>
          </Button>
              ...
              renderItem={({ item }) =>
                ...
                <ListItem style={{ paddingLeft: 0, marginLeft: 0, backgroundColor: '#FFF', opacity: 1, borderColor: 'rgba(0,0,0,0.05)', borderBottomWidth: 1 }} onPress={() => this.DietDetails(item)} >
                  ...
                  <Body style={{ paddingLeft: 0, marginLeft: 0 }}>
                    ...
                    <Text note>
                      <Ionicons name="ios-flame" /> {item.diet_calories} {Strings.ST45}
                    </Text>
                  </Body>
                  <Right>
                    <Text note>
